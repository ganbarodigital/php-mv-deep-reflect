...
/**
 * does the context have all our named parameters?
 */
class HasParametersCalled
{
     ...
     *
     * @param  array $names
     *         the list of parameters to check for
     * @param  PhpParameterContainer $context
     *         the context to examine
     * @return bool
     *         - `true` if all the parameters are in `$context`
     *         - `false` otherwise
     */
    public static function check(array $names, PhpParameterContainer $context) : bool
    {
        $params = GetAllParameters::from($context);
        return count(array_intersect(array_keys($params), $names)) == count($names);
    }
}